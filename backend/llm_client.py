...
api_key = os.getenv(
    "OPENAI_API_KEY",
    os.getenv("DEEPSEEK_API_KEY", cfg.get("api_keys", {}).get("openai_key", ""))
)
base_url = (
    os.getenv("OPENAI_API_BASE_URL")
    or os.getenv("OPENAI_BASE_URL")
    or os.getenv("DEEPSEEK_BASE_URL")
    or cfg.get("openai", {}).get("base_url", "")
)
model = os.getenv("OPENAI_MODEL") or os.getenv("DEEPSEEK_MODEL") or cfg.get("openai", {}).get("model", "claude-sonnet-4-6")
...
reasoning_effort = os.getenv("DEEPSEEK_REASONING_EFFORT") or "high"
enable_thinking = (os.getenv("DEEPSEEK_ENABLE_THINKING", "true").strip().lower() in {"1", "true", "yes", "on"})
...
def llm_request(
    ...
    if use_claude:
        ...
        if model.startswith("deepseek-") and "flash" not in model.lower():
