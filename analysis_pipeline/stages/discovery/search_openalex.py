...
def _format_result(work: dict) -> dict:
    ...
    pdf_urls = []
    landing_urls = []
    for location in [primary, best_oa, *(work.get("locations") or [])]:
        ...
        if landing_url and landing_url not in landing_urls:
    ...
    return {
        ...
        "oa_url": open_access.get("oa_url") or best_oa.get("landing_page_url") or "",
...
def search(
    ...
    try:
        per_page = max(1, min(int(max_results or 1), 200))
