...
BILINGUAL_KEYWORD_EXPANSIONS: dict[str, list[str]] = {
    "储能": [
        "energy storage",
        "battery storage",
...
class FilterGroup:
    ...
    def __post_init__(self) -> None:
        expanded: list[str] = []
        provided_keywords = [str(kw).strip() for kw in self.keywords if str(kw).strip()]
        provided_expanded = [str(kw).strip() for kw in self.expanded_keywords if str(kw).strip()]
        original_keywords = [str(kw).strip() for kw in (self.input_keywords or provided_keywords) if str(kw).strip()]
        self.input_keywords = original_keywords
        for kw in original_keywords:
            value = kw.strip()
            if not value:
                continue
            expanded.append(value)
            expanded.extend(BILINGUAL_KEYWORD_EXPANSIONS.get(value, []))
            if value == "轨迹控制":
                expanded.extend(
                    [
                        "trajectory control",
        ...
        expanded.extend(provided_expanded)
        expanded.extend(kw for kw in provided_keywords if kw.lower() not in original_set)
        seen: set[str] = set()
        self.keywords = []
        for kw in expanded:
            key = kw.lower()
        ...
        self.expanded_keywords = [kw for kw in self.keywords if kw not in self.input_keywords]

    def add_expanded_keywords(self, keywords: list[str]) -> list[str]:
        """Append externally generated expansion terms and return the terms that were added."""
        known = {kw.lower() for kw in self.keywords}
        input_keywords = {kw.lower() for kw in self.input_keywords}
        expanded = {kw.lower() for kw in self.expanded_keywords}
        added: list[str] = []
        for raw in keywords:
            kw = str(raw).strip()
            if not kw:
            ...
            self.keywords.append(kw)
            added.append(kw)
            if key not in input_keywords and key not in expanded:
                expanded.add(key)
                self.expanded_keywords.append(kw)
        return added
