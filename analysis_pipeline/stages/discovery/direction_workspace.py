...
def build_direction_workspace(
    ...
    discovery_dir = ensure_dir(output_dir / "01_discovery")
    directions_root = ensure_dir(discovery_dir / "directions")
    ...
    missing_assignments = sorted(selected_ids - set(assignments))
    if missing_assignments:
        raise RuntimeError(
            "Final direction classification did not assign every selected PDF: "
    ...
    save_json(discovery_dir / "direction_workspace_manifest.json", [str(path.resolve()) for path in created])
    assigned_workspace_ids_list = [
        str(paper.get("candidate_id"))
        for path in created
        for paper in load_json(path / "assigned_papers.json").get("papers", [])
    ]
    duplicate_workspace_ids = sorted(
        candidate_id
        for candidate_id, count in Counter(assigned_workspace_ids_list).items()
        if candidate_id and count > 1
    )
    if duplicate_workspace_ids:
        raise RuntimeError(
            "Direction workspace assigned selected PDFs more than once: "
            + ", ".join(duplicate_workspace_ids)
        )
    assigned_workspace_ids = set(assigned_workspace_ids_list)
    missing_workspace = sorted(selected_ids - assigned_workspace_ids)
    if missing_workspace:
        raise RuntimeError(
            "Direction workspace is missing selected PDFs after assignment: "
...
def load_direction_dirs(output_dir: Path) -> list[Path]:
    root = output_dir / "01_discovery" / "directions"
    if not root.exists():
        return []
